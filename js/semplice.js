...
(function ($) {
	...
	/* site transitions */
	var transitions = $('body').hasClass('no-transitions');
    ...
    $(document).ready(function () {   
        ...
        if($('#fullscreen-cover').length) {
            /* get cover and start nprogress if transitions enabled, otherwise skip */
			if(!transitions) {
				NProgress.start();
				cover();
			} else {
        ...
        function headline() {
            ...
            $('.cover-headline').transition({ opacity: 1, delay: 300 }, 800, 'ease' );
        }
		...
		$(document).on('click', '.close-project-panel, .close-panel', function() {
            ...
            $('#project-panel-header').slideUp( 600, 'easeInOutExpo', function () {
				...
				/* is sticky? */
				if(!$('header').hasClass('non-sticky-nav')) {
		...
		if(!transitions) {
			$('.logo a, .title h1 a, .title a, .thumb a, li a, h2 a, a.more-link, .meta a, .project-panel-link, #fullscreen-cover a, .fwt-link, .secondary a, .featured a, #fullscreen-menu a').click(function (a) {
