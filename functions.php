...
function load_custom_frontend_scripts() {
	...
	// Fancybox CSS
	wp_register_style('jquery-fancy-box-css', get_template_directory_uri() . '/css/fancy-box.css');
	wp_enqueue_style('jquery-fancy-box-css');
	...
	if(get_field('minify', 'options') === 'disabled') {
		...
		wp_register_script('jquery-fancy-box-js', get_template_directory_uri() . '/js/jquery.fancybox-1.3.4.pack.js', '', '', true);
		wp_enqueue_script('jquery-fancy-box-js');
...
// content editor shortcodes
require get_template_directory() . '/content-editor/shortcodes.php';
...
function custom_admin_bar() {
	...
	$args = array(
		...
		'href'   => '/wp-admin/admin.php?page=acf-options-theme-options',
	);
